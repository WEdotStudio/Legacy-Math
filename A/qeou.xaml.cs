...
namespace Math10
{
    ...
    public sealed partial class qeou : Page
    {
        public qeou()
        {
        ...
        private async void Solve(object sender, RoutedEventArgs e)
        {
            ...
            {
                MessageDialog messageDialog = new MessageDialog("输入为空！");
                await messageDialog.ShowAsync();
                return;
            }
            if (at.Text == "0")
            {
                MessageDialog messageDialog = new MessageDialog("二次项不得为0！");
                await messageDialog.ShowAsync();
                return;
            }
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (p_i.Visibility == Visibility.Visible)
                    {
                        p_appro.Text = "(约为 " + appro1 + "+" + appro2 + "i)";
                        n_appro.Text = "(约为 " + appro1 + "-" + appro2 + "i)";
                    }
                    else
                    {
                        p_appro.Text = "(约为 " + appro1 + "+" + appro2 + ")";
                        n_appro.Text = "(约为 " + appro1 + "-" + appro2 + ")";
                    };
                    //Finalise the answer
                    p_numer.Text = b.ToString();
                    p_ratdoni.Text = a.ToString();
